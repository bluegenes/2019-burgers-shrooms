...
file <-  file.path(snakemake@input[["sample_correlation"]])

sample_full_df <- readr::read_tsv(file,
...
print(dim(sample_full_df))

head(sample_full_df)

sample_subset_df <- subset_correlations(df = sample_full_df,
                                      cor_type = "pearson",
                                      data_type = "testing",
                                      signal_type = "signal")

print(dim(sample_subset_df))

head(sample_subset_df)

sample_full_corr_gg <- plot_correlation_summary(df = sample_subset_df)
sample_full_corr_gg
...
file <-  file.path(snakemake@output[["sample_correlation_summary"]])
sample_summary_df <- readr::read_tsv(file,
                                   col_types = readr::cols(
                                       .default = readr::col_character(),
                                       num_comp = readr::col_integer(),
...
head(sample_summary_df)

sample_subset_summary_df <- process_summary_df(summary_df = sample_summary_df,
                                             select_sample_types = select_cancer_types,
                                             cor_type = 'pearson',
                                             data_type = 'testing',
                                             signal_type = 'signal')

sample_select_cancertype_gg <- plot_subset_summary(sample_subset_summary_df, palette = mean_cor_palette)
sample_select_cancertype_gg

sample_capacity_gain_df <- process_capacity(sample_summary_df, select_cancer_types)
sample_select_cancertype_capactity_gain_gg <- plot_capacity_difference(sample_capacity_gain_df)

sample_select_cancertype_capactity_gain_gg
...
main_plot <- (
    cowplot::plot_grid(
        sample_full_corr_gg + theme(legend.position = 'none'),
        sample_select_cancertype_gg + theme(legend.position = "none"),
        sample_select_cancertype_capactity_gain_gg + theme(legend.position = 'none'),
#        gtex_select_tissuetype_capactity_gain_gg + theme(legend.position = 'none'),
        labels = c("a", "b", "c", "d"), # these won't be right
        ncol = 2,
        nrow = 2
    )
)
